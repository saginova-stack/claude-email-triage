...
[`gog`](https://github.com/steipete/gogcli) (by Peter Steinberger) is a fast, script-friendly CLI for Google Workspace — Gmail, Calendar, Drive, Docs, and more. Claude calls `gog` commands to read and act on your email.
...
Docs and full command reference: https://github.com/steipete/gogcli
...
## Part 5: Set up the skill and config files (5 min)

This repo separates **logic** (`skill/SKILL.md` — the agent's decision-making) from **config** (`config.example.md` — your personal accounts, VIPs, tone, etc.). You customize the config once. Future updates to the skill won't overwrite your settings.
...
### 5.3 Create your personal config

Copy the example config and edit it to match your accounts, VIPs, tone, and meeting preferences:

```bash
cp config.example.md ~/email-triage/config.md
open -e ~/email-triage/config.md
```

**This is the only file you need to customize.** Edit the sections inside — accounts, VIPs, tone, meeting preferences, project context. The skill reads this file at the start of every run.

### 5.4 Drop the skill into place
...
Do NOT edit this file. It contains only the agent's logic. Your personal config lives in `~/email-triage/config.md` (from 5.3). When future updates to the skill land, you can drop in the new `SKILL.md` without losing your settings.
...
**Update your personal settings** (accounts, VIPs, tone, meeting prefs):
1. Edit `~/email-triage/config.md`
2. Save — next run picks it up

**Update the agent logic** (pull improvements from the repo):
1. `cp skill/SKILL.md ~/.claude/scheduled-tasks/email-triage/SKILL.md` — overwrites the logic
2. Your config at `~/email-triage/config.md` is untouched

If you set up multi-schedule with symlinks (Part 9), you only need to replace the canonical `SKILL.md`; the symlinks pick it up.
